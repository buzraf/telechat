...
function initDB($db) {
    ...
    // Auto-fix broken PostgreSQL sequences or force reset
    if ($pg) {
        $reset = getenv('RESET_DB') === 'true';
        if (!$reset) {
            // Check if users table has broken id column (no sequence)
            try {
                $col = $db->query("SELECT column_default FROM information_schema.columns WHERE table_name='users' AND column_name='id'")->fetch(PDO::FETCH_ASSOC);
                if ($col && (empty($col['column_default']) || $col['column_default'] === null)) {
                    $reset = true; // id has no default sequence — need to recreate
                }
            } catch (Exception $e) { $reset = true; }
        }
        if ($reset) {
            $db->exec("DROP TABLE IF EXISTS events CASCADE");
            $db->exec("DROP TABLE IF EXISTS gifts_sent CASCADE");
            $db->exec("DROP TABLE IF EXISTS gifts_inventory CASCADE");
            $db->exec("DROP TABLE IF EXISTS coins CASCADE");
    ...
    if ($pg) {
        ...
        // Global chat для PostgreSQL — используем sequence
        $db->exec("INSERT INTO chats (type, name) SELECT 'group', '🌍 TeleChat Global' WHERE NOT EXISTS (SELECT 1 FROM chats WHERE name='🌍 TeleChat Global')");
        $globalChat = $db->query("SELECT id FROM chats WHERE name='🌍 TeleChat Global' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
        if ($globalChat) {
            $gcId = $globalChat['id'];
            $db->exec("INSERT INTO chat_members (chat_id, user_id) SELECT $gcId, id FROM users ON CONFLICT DO NOTHING");
            $count = $db->query("SELECT COUNT(*) FROM messages WHERE chat_id=$gcId")->fetchColumn();
            if ($count == 0) {
                $db->exec("INSERT INTO messages (chat_id, sender_id, content, type) VALUES ($gcId, 0, '👋 Добро пожаловать в TeleChat Global!', 'system')");
            }
        }
        // Dev coins
        ...
        $db->exec("CREATE TABLE IF NOT EXISTS chats (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            type TEXT DEFAULT 'private', name TEXT DEFAULT '',
            avatar TEXT DEFAULT '', created_by INTEGER,
            last_message_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP
        )");
        $db->exec("CREATE TABLE IF NOT EXISTS chat_members (
            chat_id INTEGER, user_id INTEGER, joined_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            PRIMARY KEY (chat_id, user_id)
        )");
        $db->exec("CREATE TABLE IF NOT EXISTS messages (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        $db->exec("CREATE TABLE IF NOT EXISTS events (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            chat_id INTEGER, type TEXT NOT NULL,
            data TEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP
        )");
        $db->exec("CREATE TABLE IF NOT EXISTS coins (
        ...
        $db->exec("CREATE TABLE IF NOT EXISTS gifts_inventory (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            owner_id INTEGER NOT NULL, gift_id TEXT NOT NULL,
            from_user_id INTEGER, from_name TEXT DEFAULT '',
            message TEXT DEFAULT '', created_at DATETIME DEFAULT CURRENT_TIMESTAMP
        )");
        $db->exec("CREATE TABLE IF NOT EXISTS gifts_sent (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            chat_id INTEGER, from_user_id INTEGER NOT NULL,
...
function createEvent($db, $chatId, $type, $data) {
    try {
        ...
        $stmt->execute([(int)$chatId, $type, $json]);
        if (isPg()) {
    ...
    try {
        ...
        if ($method==='POST' && $path==='/api/auth/register') {
            ...
            try {
                ...
                // Add to global chat
                $gcRow = $db->query("SELECT id FROM chats WHERE name='🌍 TeleChat Global' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
                $gcId = $gcRow ? $gcRow['id'] : 1;
                if (isPg()) {
                    $db->prepare("INSERT INTO chat_members (chat_id, user_id) VALUES (?, ?) ON CONFLICT DO NOTHING")->execute([$gcId, $userId]);
                    $db->prepare("INSERT INTO coins (user_id, amount) VALUES (?, '1000') ON CONFLICT (user_id) DO NOTHING")->execute([$userId]);
                } else {
                    $db->prepare("INSERT OR IGNORE INTO chat_members (chat_id, user_id) VALUES (?, ?)")->execute([$gcId, $userId]);
        ...
        if ($method==='POST' && $path==='/api/auth/login') {
            ...
            $gcRow2 = $db->query("SELECT id FROM chats WHERE name='🌍 TeleChat Global' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
            $gcId2 = $gcRow2 ? $gcRow2['id'] : 1;
            if (isPg()) {
                $db->prepare("INSERT INTO chat_members (chat_id, user_id) VALUES (?, ?) ON CONFLICT DO NOTHING")->execute([$gcId2, $user['id']]);
                $db->prepare("INSERT INTO coins (user_id, amount) VALUES (?, '1000') ON CONFLICT (user_id) DO NOTHING")->execute([$user['id']]);
            } else {
                $db->prepare("INSERT OR IGNORE INTO chat_members (chat_id, user_id) VALUES (?, ?)")->execute([$gcId2, $user['id']]);
...
@keyframes particleFloat{0%{transform:translateY(100vh) rotate(0deg);opacity:0}5%{opacity:1}95%{opacity:1}100%{transform:translateY(-120px) rotate(360deg);opacity:0}}
    ...
    <div style={{minHeight:'100vh',background:'linear-gradient(135deg,#0a0a12,#0d0d1a,#13131f)',display:'flex',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden'}}>
      ...
      <div style={{
        ...
        <div style={{padding:'36px 36px 24px',textAlign:'center',background:'linear-gradient(180deg,rgba(124,58,237,0.15),transparent)'}}>
            ...
            <svg width="36" height="36" viewBox="0 0 24 24" fill="none"><path d="M22 2L11 13" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M22 2L15 22L11 13L2 9L22 2Z" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </div>
          <div style={{fontSize:28,fontWeight:900,background:'linear-gradient(135deg,#a78bfa,#e879f9)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',letterSpacing:'-0.5px'}}>TeleChat</div>
          <div style={{color:'#4a5568',fontSize:13,marginTop:4,letterSpacing:'0.05em'}}>Общайтесь без границ</div>
        </div>
        {/* Tabs */}
        ...
        <form onSubmit={submit} style={{padding:'24px 28px 32px',display:'flex',flexDirection:'column',gap:14}}>
          ...
          <button type="submit" disabled={loading} style={{
            ...
            {loading ? <><div style={{width:18,height:18,border:'2px solid rgba(255,255,255,0.3)',borderTopColor:'#fff',borderRadius:'50%',animation:'spin 0.8s linear infinite'}}/> Загрузка...</> : (tab==='login' ? 'Войти в TeleChat' : 'Создать аккаунт')}
          </button>
        </form>
      </div>
    </div>
...
function GiftShopModal({ user, onClose, activeChatId, chatUser }) {
  ...
  const handleBuy = async (gift) => {
    ...
    } else showToast(r.error||'Ошибка покупки', false);
  };
  ...
  const handleSend = async () => {
    ...
    else showToast(r.error||'Ошибка отправки', false);
  };
      ...
      <div style={{
        ...
        <div style={{padding:'20px 24px',background:'linear-gradient(135deg,rgba(124,58,237,0.3),rgba(168,85,247,0.1))',borderBottom:'1px solid rgba(124,58,237,0.2)',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <div style={{fontSize:24,filter:'drop-shadow(0 4px 8px rgba(124,58,237,0.5))'}}>
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="2"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
            </div>
            <div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <div style={{background:'rgba(251,191,36,0.15)',border:'1px solid rgba(251,191,36,0.4)',borderRadius:12,padding:'8px 16px',display:'flex',alignItems:'center',gap:6}}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="#fbbf24"><circle cx="12" cy="12" r="10"/><text x="12" y="16" textAnchor="middle" fontSize="12" fill="#1a1a2e" fontWeight="bold">$</text></svg>
        ...
        <div style={{display:'flex',padding:'12px 24px 0',gap:4,flexShrink:0}}>
          {['shop','inventory'].map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{
              ...
              transition:'all 0.2s'
            }}>{t==='shop' ? 'Магазин' : `Инвентарь (${inventory.length})`}</button>
          ))}
        </div>
        {/* Filter */}
        {tab==='shop' && (
        ...
        <div style={{flex:1,overflowY:'auto',padding:20}}>
          {tab==='shop' ? (
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))',gap:16}}>
              {filtered.map(gift=>(
                <div key={gift.id} style={{
                  ...
                  <div style={{color:'#fbbf24',fontWeight:800,fontSize:16}}>{gift.price} монет</div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))',gap:16}}>
              {inventory.length===0 ? (
                <div style={{color:'#64748b',textAlign:'center',gridColumn:'1/-1',padding:40,fontSize:16}}>Инвентарь пуст — купи что-нибудь в магазине!</div>
          ...
          <div style={{display:'flex',justifyContent:'center'}}>
            <div style={{position:'relative',cursor:'pointer'}} onClick={()=>fileRef.current?.click()}>
              ...
              <div style={{position:'absolute',inset:0,borderRadius:'50%',background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',opacity:0,transition:'opacity 0.2s'}}
                ...
                {avatarLoading ? <div style={{width:20,height:20,border:'2px solid rgba(255,255,255,0.3)',borderTopColor:'#fff',borderRadius:'50%',animation:'spin 0.8s linear infinite'}}/> : <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><path d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg>}
              </div>
      ...
      <div style={{padding:'16px 16px 12px',background:'rgba(124,58,237,0.05)',borderBottom:'1px solid rgba(124,58,237,0.1)'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
          ...
          <div style={{display:'flex',gap:6}}>
            <button onClick={onShowShop} title="Магазин" style={{background:'rgba(124,58,237,0.12)',border:'1px solid rgba(124,58,237,0.25)',borderRadius:10,width:36,height:36,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',transition:'all 0.2s'}}
              onMouseEnter={e=>{e.currentTarget.style.background='rgba(124,58,237,0.3)';e.currentTarget.style.transform='scale(1.1)';}}
              onMouseLeave={e=>{e.currentTarget.style.background='rgba(124,58,237,0.12)';e.currentTarget.style.transform='';}}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="2"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
            </button>
            <button onClick={onNewChat} title="Новый чат" style={{background:'rgba(124,58,237,0.15)',border:'1px solid rgba(124,58,237,0.3)',borderRadius:10,width:36,height:36,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'#a78bfa',transition:'all 0.2s'}}
              onMouseEnter={e=>{e.currentTarget.style.background='rgba(124,58,237,0.3)';e.currentTarget.style.transform='scale(1.1)';}}
              onMouseLeave={e=>{e.currentTarget.style.background='rgba(124,58,237,0.15)';e.currentTarget.style.transform='';}}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
            </button>
          </div>
        </div>
        {/* Search */}
        <div style={{position:'relative'}}>
          <span style={{position:'absolute',left:12,top:'50%',transform:'translateY(-50%)',color:'#64748b'}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#64748b" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg></span>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'4px 8px'}}>
          {filtered.length === 0 && (
            <div style={{color:'#64748b',textAlign:'center',padding:'40px 20px',fontSize:14,lineHeight:1.8}}>
              <div style={{marginBottom:16,opacity:0.4}}><svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="1.5"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg></div>
              Нет чатов. Нажми кнопку редактирования чтобы начать!
            </div>
          )}
          {filtered.map((chat,i) => {
