<?php
// ============================================================
// TeleChat v5.1 — Full Telegram Clone
// One-file PHP app: Backend + Frontend + SQLite/PostgreSQL DB
// ============================================================
...
// OPTIONS preflight — всегда отвечаем быстро
if ($method === 'OPTIONS') {
...
// Для API запросов — JSON заголовки
...
function dbNow() {
    return defined('DB_TYPE') && DB_TYPE === 'pgsql' ? 'NOW()' : "datetime('now')";
}
function dbAutoInc() {
    return defined('DB_TYPE') && DB_TYPE === 'pgsql' ? 'SERIAL PRIMARY KEY' : 'INTEGER PRIMARY KEY AUTOINCREMENT';
}
function dbLastId($db, $table = '', $col = 'id') {
...
function createEvent($db, $chatId, $type, $data) {
    ...
    if (defined('DB_TYPE') && DB_TYPE === 'pgsql') {
        $db->exec("DELETE FROM events WHERE created_at < NOW() - INTERVAL '5 minutes'");
    } else {
        $db->exec("DELETE FROM events WHERE created_at < datetime('now', '-5 minutes')");
    }
}
function getLastInsertId($db, $table, $col = 'id') {
    if (defined('DB_TYPE') && DB_TYPE === 'pgsql') {
        return $db->lastInsertId("{$table}_{$col}_seq");
    }
    return $db->lastInsertId();
...
if ($method==='GET' && $path==='/api/status') {
    ...
    echo json_encode(['status'=>'ok','db'=>$dbType,'users'=>(int)$users,'messages'=>(int)$msgs,'version'=>'TeleChat v5.0']);
    exit;
}
...
if ($method==='POST' && $path==='/api/auth/login') {
    ...
    $isPg2 = defined('DB_TYPE') && DB_TYPE === 'pgsql';
    if ($isPg2) {
...
if ($method==='POST' && $path==='/api/chats') {
    ...
    foreach ($memberIds as $mid) {
        if ((int)$mid !== $user['id']) {
            if ($isPg) {
                $db->prepare("INSERT INTO chat_members (chat_id, user_id) VALUES (?,?) ON CONFLICT DO NOTHING")->execute([$chatId, (int)$mid]);
            } else {
                $db->prepare("INSERT OR IGNORE INTO chat_members (chat_id, user_id) VALUES (?,?)")->execute([$chatId, (int)$mid]);
            }
        }
    }
...
if ($method==='POST' && $path==='/api/shop/buy') {
    ...
    if ($isPg) {
        $db->prepare("INSERT INTO coins (user_id,amount) VALUES (?,?) ON CONFLICT (user_id) DO UPDATE SET amount=EXCLUDED.amount")->execute([$user['id'], $newBalance]);
    } else {
        $db->prepare("INSERT OR REPLACE INTO coins (user_id,amount) VALUES (?,?)")->execute([$user['id'], $newBalance]);
    }
...
if ($method==='POST' && $path==='/api/shop/send') {
    ...
    if ($chatId) {
        ...
        if ($isPg) {
            $gstmt = $db->prepare("INSERT INTO messages (chat_id,sender_id,content,type) VALUES (?,?,?,'gift') RETURNING id");
            $gstmt->execute([$chatId,$user['id'],$giftContent]);
            $msgId = (int)$gstmt->fetchColumn();
            $db->prepare("UPDATE chats SET last_message_at=NOW() WHERE id=?")->execute([$chatId]);
        } else {
...
const api = {
  ...
  get: async (url) => {
    try {
      const r = await fetch(BASE + url, {headers: api.headers()});
      if (!r.ok) {
        const err = await r.json().catch(() => ({error: 'Ошибка сервера ' + r.status}));
        return err;
      }
      return r.json();
    } catch(e) {
  ...
  del: async (url) => {
    try {
      const r = await fetch(BASE + url, {method: 'DELETE', headers: api.headers()});
      if (!r.ok) {
        const err = await r.json().catch(() => ({error: 'Ошибка сервера ' + r.status}));
        return err;
      }
  ...
  const submit = async(e)=>{
    ...
    try {
      ...
      const body = tab==='login'
        ...
        : {email: email.trim(), password, username: username.trim(), display_name: (displayName||username).trim()};
      ...
      let d;
      try { d = await r.json(); } catch(je) { throw new Error('Сервер вернул некорректный ответ'); }
      ...
      setError(err.message || 'Ошибка соединения. Проверьте интернет.');
      setLoading(false);
    }
  };
