    ...
    <div style={{minHeight:'100vh',width:'100vw',display:'flex',alignItems:'flex-start',justifyContent:'center',background:'linear-gradient(135deg,#06060f 0%,#0d0720 50%,#06060f 100%)',position:'relative',overflowY:'auto',overflowX:'hidden',paddingTop:40,paddingBottom:40}}>
      ...
      <div className="scale-in" style={{width:'100%',maxWidth:480,margin:'0 16px',position:'relative',zIndex:1}}>
        ...
        <div style={{textAlign:'center',marginBottom:20}}>
          <div className="glow-pulse" style={{width:72,height:72,borderRadius:22,background:'linear-gradient(135deg,#7c3aed,#4c1d95)',boxShadow:'0 12px 36px rgba(124,58,237,.5)',display:'inline-flex',alignItems:'center',justifyContent:'center',marginBottom:12}}>
            <Ic name="msg" size={38} color="white" strokeWidth={1.5}/>
          </div>
          <h1 style={{fontSize:30,fontWeight:900,background:'linear-gradient(135deg,#c4b5fd,#fff 50%,#a78bfa)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',letterSpacing:'-1px',marginBottom:4}}>TeleChat</h1>
          <p style={{color:'rgba(255,255,255,.35)',fontSize:13,fontWeight:500}}>Secure. Fast. Beautiful.</p>
        </div>

        {/* Card */}
        <div style={{background:'rgba(255,255,255,.03)',backdropFilter:'blur(24px)',border:'1px solid rgba(124,58,237,.2)',borderRadius:28,padding:'28px 32px',boxShadow:'0 24px 80px rgba(0,0,0,.6),inset 0 1px 0 rgba(255,255,255,.08)'}}>
          {/* Tabs */}
          <div style={{display:'flex',background:'rgba(0,0,0,.3)',borderRadius:16,padding:4,marginBottom:20,position:'relative'}}>
    ...
    <div>
      ...
      <div style={{
        ...
        <input value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} type={type} autoComplete={autoComplete}
          ...
          style={{flex:1,padding:'14px 16px',background:'transparent',color:'#fff',fontSize:16,lineHeight:1.5,height:54}}/>
