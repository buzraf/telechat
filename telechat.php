<?php
/**
 * TeleChat v4.0 — PostgreSQL + SQLite fallback
 * Один PHP файл: бэкенд + фронтенд + БД
 * Данные НЕ теряются при редеплое!
 */

// Показываем все ошибки в лог (не в браузер)
ini_set('display_errors', 0);
ini_set('log_errors', 1);
error_reporting(E_ALL);

// ─── Database Connection (PostgreSQL primary, SQLite fallback) ─────────────────
function getDB(): PDO {
    static $pdo = null;
    if ($pdo) return $pdo;

    // Railway PostgreSQL — поддержка обоих форматов URL
    ...
    if ($dbUrl) {
        // Нормализуем схему: postgresql:// → pgsql://
        ...
        // sslmode=require для Railway, allow для локального
        $sslmode = (strpos($host, 'railway') !== false || strpos($host, 'rlwy') !== false) 
                   ? 'require' : 'prefer';
        ...
        try {
            ...
            error_log("PostgreSQL error: " . $e->getMessage() . " — falling back to SQLite");
            $dbUrl = '';
        }
    }
    ...
    if (!$dbUrl || !$pdo) {
        ...
        $pdo = new PDO('sqlite:' . $dir . '/telechat.db', null, null, [
            PDO::ATTR_ERRMODE            => PDO::ERRMODE_EXCEPTION,
            PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC,
        ]);
    ...
    ensureGlobalChat($pdo);
    return $pdo;
}

function initPostgres(PDO $pdo): void {
    $pdo->exec("
...
define('GLOBAL_CHAT_ID', 'telechat-global-000-0000-000000000000');
define('JWT_SECRET', getenv('JWT_SECRET') ?: 'TeleChat_Ultra_Secret_2024_xK9mP2_secure');
define('MAX_FILE_SIZE', 50 * 1024 * 1024);

$dbDir = getenv('DATA_DIR') ?: __DIR__ . '/data';
if (!is_dir($dbDir)) @mkdir($dbDir, 0777, true);
define('UPLOAD_DIR', $dbDir . '/uploads');
if (!is_dir(UPLOAD_DIR)) @mkdir(UPLOAD_DIR, 0777, true);

function ensureGlobalChat(PDO $db): void {
    $isPostgres = ($db->getAttribute(PDO::ATTR_DRIVER_NAME) === 'pgsql');
    $now = (int)(microtime(true) * 1000);
    if ($isPostgres) {
        $db->prepare("INSERT INTO chats (id,type,name,description,avatar,created_by,created_at)
            VALUES (?,?,?,?,?,?,?)
            ON CONFLICT (id) DO NOTHING")
           ->execute([GLOBAL_CHAT_ID,'group','🌍 TeleChat Global',
             'Общий чат для всех пользователей TeleChat 👋',
             'https://ui-avatars.com/api/?name=TeleChat+Global&background=7c3aed&color=fff&bold=true&size=200',
             'system', $now]);
        // Добавить всех существующих пользователей
        $db->exec("INSERT INTO chat_members (chat_id,user_id,role,joined_at)
            SELECT '".GLOBAL_CHAT_ID."', id, 'member', $now FROM users
            ON CONFLICT (chat_id,user_id) DO NOTHING");
    } else {
        $db->exec("INSERT OR IGNORE INTO chats (id,type,name,description,avatar,created_by,created_at) VALUES (
            '".GLOBAL_CHAT_ID."','group','🌍 TeleChat Global',
            'Общий чат для всех пользователей TeleChat 👋',
            'https://ui-avatars.com/api/?name=TeleChat+Global&background=7c3aed&color=fff&bold=true&size=200',
            'system',$now)");
        $db->exec("INSERT OR IGNORE INTO chat_members (chat_id,user_id,role,joined_at)
            SELECT '".GLOBAL_CHAT_ID."', id, 'member', $now FROM users");
    }
    // Приветственное сообщение только если пусто
    $check = $db->query("SELECT COUNT(*) as c FROM messages WHERE chat_id='".GLOBAL_CHAT_ID."'")->fetch();
    if ((int)$check['c'] === 0) {
        $db->prepare("INSERT INTO messages (id,chat_id,sender_id,type,content,file_url,file_name,file_size,file_mime,reply_to,created_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?)")
           ->execute(['msg-welcome-001', GLOBAL_CHAT_ID, 'system', 'text',
             '👋 Добро пожаловать в TeleChat Global! Это общий чат для всех пользователей. Общайтесь и знакомьтесь! 🚀',
             '', '', 0, '', '', $now]);
    }
}

function addUserToGlobalChat(string $userId, PDO $db): void {
    $isPostgres = ($db->getAttribute(PDO::ATTR_DRIVER_NAME) === 'pgsql');
    $now = (int)(microtime(true) * 1000);
    if ($isPostgres) {
        $db->prepare("INSERT INTO chat_members (chat_id,user_id,role,joined_at) VALUES (?,?,'member',?)
            ON CONFLICT (chat_id,user_id) DO NOTHING")
           ->execute([GLOBAL_CHAT_ID, $userId, $now]);
    } else {
        $db->prepare("INSERT OR IGNORE INTO chat_members (chat_id,user_id,role,joined_at) VALUES (?,?,'member',?)")
           ->execute([GLOBAL_CHAT_ID, $userId, $now]);
    }
    // Сообщение о входе
    ...
    $user = $stmt->fetch();
    if ($user) {
        $mid = 'msg-join-' . time() . '-' . substr($userId, 0, 8);
        $db->prepare("INSERT INTO messages (id,chat_id,sender_id,type,content,file_url,file_name,file_size,file_mime,reply_to,created_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?)")
           ->execute([$mid, GLOBAL_CHAT_ID, 'system', 'text',
             '➕ ' . $user['display_name'] . ' присоединился к TeleChat Global!',
             '', '', 0, '', '', $now]);
        pushEventToChat(GLOBAL_CHAT_ID, 'message:new', ['chatId' => GLOBAL_CHAT_ID], $userId);
    }
}
...
function formatMessage(array $m): array {
    ...
    return [
        ...
        'sender_name'    => $isSystem ? 'TeleChat' : ($m['sender_name'] ?? ''),
        'sender_avatar'  => $isSystem ? 'https://ui-avatars.com/api/?name=TC&background=7c3aed&color=fff&bold=true' : ($m['sender_avatar'] ?? ''),
        'sender_username'=> $isSystem ? 'system' : ($m['sender_username'] ?? ''),
        'is_system'      => $isSystem,
    ];
}
...
function pushEvent(string $userId, string $type, array $payload): void {
    ...
    $db->prepare("INSERT INTO events (user_id,type,payload,created_at) VALUES (?,?,?,?)")
       ->execute([$userId, $type, json_encode($payload, JSON_UNESCAPED_UNICODE), (int)(microtime(true)*1000)]);
    // Чистим старые события (оставляем последние 500)
...
// ─── Avatar upload ────────────────────────────────────────────────────────────
function handleAvatarUpload(array $u, PDO $db): void {
    if (empty($_FILES['avatar'])) error_out('No file');
    $file = $_FILES['avatar'];
    if ($file['error'] !== UPLOAD_ERR_OK) error_out('Upload error');
    if ($file['size'] > 5 * 1024 * 1024) error_out('Max 5MB');
    if (!str_starts_with($file['type'], 'image/')) error_out('Images only');
    $data = file_get_contents($file['tmp_name']);
    $b64  = 'data:' . $file['type'] . ';base64,' . base64_encode($data);
    $db->prepare("UPDATE users SET avatar=? WHERE id=?")->execute([$b64, $u['id']]);
...
if (strpos($path, '/api/') === 0) {
    ...
    if ($path === '/api/status' && $method === 'GET') {
        try {
            ...
            $chats = $db->query("SELECT COUNT(*) as c FROM chats")->fetch()['c'];
            json_out([
                'status'   => 'ok',
                'db'       => $driver === 'pgsql' ? 'PostgreSQL ✅' : 'SQLite (fallback)',
                'users'    => (int)$users,
                'messages' => (int)$msgs,
                'chats'    => (int)$chats,
                'version'  => 'TeleChat v3.0',
    ...
    if ($path === '/api/auth/register' && $method === 'POST') {
        ...
        $un    = strtolower(trim($body['username'] ?? ''));
        $dn    = trim($body['displayName'] ?? '');
        $pw    = $body['password'] ?? '';
        if (!$email || !$un || !$dn || !$pw) error_out('All fields required');
        ...
        $s2 = $db->prepare("SELECT id FROM users WHERE username=?"); $s2->execute([$un]);
        if ($s2->fetch()) error_out('Username taken', 409);
        $hash = password_hash($pw, PASSWORD_BCRYPT, ['cost' => 11]);
        $uid  = uuid(); $now = (int)(microtime(true)*1000); $av = avatar($dn);
        $db->prepare("INSERT INTO users (id,email,username,display_name,password_hash,avatar,status,bio,last_seen,created_at) VALUES (?,?,?,?,?,?,'offline','',0,?)")
           ->execute([$uid, $email, $un, $dn, $hash, $av, $now]);
        addUserToGlobalChat($uid, $db);
    ...
    if ($path === '/api/auth/login' && $method === 'POST') {
        $email = strtolower(trim($body['email'] ?? '')); $pw = $body['password'] ?? '';
        ...
        $db->prepare("UPDATE users SET status='online',last_seen=? WHERE id=?")->execute([(int)(microtime(true)*1000), $user['id']]);
        addUserToGlobalChat($user['id'], $db);
    ...
    if ($path === '/api/users/avatar' && $method === 'POST') {
        ...
        handleAvatarUpload($u, $db);
    }

    // PUT /api/users/profile
    if ($path === '/api/users/profile' && $method === 'PUT') {
        $u  = requireAuth(); $db = getDB(); $sets = []; $vals = [];
        if (isset($body['displayName'])) { $sets[] = 'display_name=?'; $vals[] = trim($body['displayName']); }
        if (isset($body['bio']))         { $sets[] = 'bio=?';           $vals[] = trim($body['bio']); }
        if (isset($body['username'])) {
            $nu = strtolower(trim($body['username']));
    ...
    if ($path === '/api/users/status' && $method === 'PUT') {
        ...
        $db->prepare("UPDATE users SET status=?,last_seen=? WHERE id=?")->execute([$st, (int)(microtime(true)*1000), $u['id']]);
    ...
    // Поиск через @ — @username ищет только по нику
    // Без @ — ищет по имени, нику и email
    if ($path === '/api/users/search' && $method === 'GET') {
        $u   = requireAuth(); $db = getDB();
        $raw = trim($_GET['q'] ?? '');
        if (strlen($raw) < 1) json_out(['users' => []]);

        // Если запрос начинается с @ — поиск только по username
        if ($raw[0] === '@') {
            $q    = '%' . substr($raw, 1) . '%';
            $stmt = $db->prepare("SELECT * FROM users WHERE username LIKE ? AND id!=? LIMIT 30");
            $stmt->execute([$q, $u['id']]);
        } else {
            $q    = '%' . $raw . '%';
    ...
    // POST /api/upload — загрузка файлов (base64 в БД или файл)
    if ($path === '/api/upload' && $method === 'POST') {
        ...
        // Для картинок — сохраняем как base64 в БД, для остальных — на диск
        $mime = $file['type'];
        if (str_starts_with($mime, 'image/') && $file['size'] <= 2 * 1024 * 1024) {
            $data    = file_get_contents($file['tmp_name']);
    ...
    if ($path === '/api/chats/private' && $method === 'POST') {
        ...
        $ex = $db->prepare("SELECT c.id FROM chats c
        ...
        $cid = uuid(); $now = (int)(microtime(true)*1000);
    ...
    if ($path === '/api/chats/group' && $method === 'POST') {
        ...
        $cid  = uuid(); $now = (int)(microtime(true)*1000); $av = avatar($name);
        ...
        foreach (($body['memberIds'] ?? []) as $mid) {
            if ($mid !== $u['id']) {
                $isPostgres = ($db->getAttribute(PDO::ATTR_DRIVER_NAME) === 'pgsql');
                if ($isPostgres) {
                    $db->prepare("INSERT INTO chat_members (chat_id,user_id,role,joined_at) VALUES (?,?,'member',?) ON CONFLICT DO NOTHING")->execute([$cid, $mid, $now]);
                } else {
    ...
    if (preg_match('#^/api/chats/([^/]+)/messages$#', $path, $m) && $method === 'GET') {
        ...
        $stmt   = $db->prepare("SELECT m.*,u.display_name as sender_name,u.avatar as sender_avatar,u.username as sender_username
            FROM messages m LEFT JOIN users u ON m.sender_id=u.id
            WHERE m.chat_id=? AND m.created_at<? ORDER BY m.created_at DESC LIMIT ?");
    ...
    if (preg_match('#^/api/chats/([^/]+)/messages$#', $path, $m) && $method === 'POST') {
        ...
        $mid = uuid(); $now = (int)(microtime(true)*1000);
        $db->prepare("INSERT INTO messages (id,chat_id,sender_id,type,content,file_url,file_name,file_size,file_mime,reply_to,edited,deleted,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,0,0,?)")
    ...
    if (preg_match('#^/api/messages/([^/]+)$#', $path, $m) && $method === 'PUT') {
        ...
        $stmt2 = $db->prepare("SELECT m.*,u.display_name as sender_name,u.avatar as sender_avatar,u.username as sender_username
            FROM messages m LEFT JOIN users u ON m.sender_id=u.id WHERE m.id=?");
    ...
    if (preg_match('#^/api/messages/([^/]+)$#', $path, $m) && $method === 'DELETE') {
        ...
        if (!$msg) error_out('Not found', 404);
        if ($msg['sender_id'] !== $u['id']) error_out('Forbidden', 403);
        $db->prepare("UPDATE messages SET deleted=1,content='Сообщение удалено' WHERE id=?")->execute([$mid]);
    ...
    if ($path === '/api/events' && $method === 'GET') {
        ...
        $db->prepare("UPDATE users SET status='online',last_seen=? WHERE id=?")->execute([(int)(microtime(true)*1000), $u['id']]);
        $timeout = 25; $start = time();
        while (true) {
    ...
    if ($path === '/api/call/signal' && $method === 'POST') {
        ...
        pushEvent($toId, 'call:' . ($body['type'] ?? ''), [
            'from'     => $u['id'],
            'fromUser' => formatUser($u),
            'signal'   => $body['signal'] ?? [],
...
/* ── File Drop Zone ── */
.drop-zone{border:2px dashed rgba(124,58,237,.3);border-radius:16px;transition:all .25s ease;}
.drop-zone.drag-over{border-color:rgba(124,58,237,.8);background:rgba(124,58,237,.08);animation:uploadPulse 1s ease infinite;}

/* ── Image Preview ── */
.media-img{border-radius:12px;max-width:280px;max-height:280px;object-fit:cover;cursor:pointer;transition:transform .2s ease,filter .2s ease;display:block;}
.media-img:hover{transform:scale(1.02);filter:brightness(1.05)}

/* ── Avatar upload ── */
.avatar-upload-btn{transition:all .25s ease;position:relative;overflow:hidden;}
.avatar-upload-btn:hover .avatar-overlay{opacity:1}
.avatar-overlay{position:absolute;inset:0;background:rgba(0,0,0,.55);display:flex;align-items:center;justify-content:center;opacity:0;transition:opacity .22s ease;border-radius:50%;}

/* ── Typing dots ── */
.typing-dot{width:6px;height:6px;border-radius:50%;background:#a78bfa;display:inline-block;margin:0 2px;animation:dot 1.2s ease-in-out infinite;}
...
.input-field:focus{border-color:rgba(124,58,237,.6)!important;box-shadow:0 0 0 3px rgba(124,58,237,.15)!important;}
...
/* ── Online dot ── */
.online-dot{width:10px;height:10px;border-radius:50%;background:#4ade80;border:2px solid #13131f;box-shadow:0 0 8px rgba(74,222,128,.6);animation:pulse 2s ease-in-out infinite;}
.offline-dot{width:10px;height:10px;border-radius:50%;background:rgba(255,255,255,.2);border:2px solid #13131f}

/* ── Gradient text ── */
.gradient-text{background:linear-gradient(135deg,#a78bfa,#7c3aed,#c4b5fd);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;animation:gradientShift 3s ease infinite;}

/* ── Glass card ── */
.glass{background:rgba(255,255,255,.04);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,.07)}

/* ── Media lightbox ── */
.lightbox{position:fixed;inset:0;z-index:10000;background:rgba(0,0,0,.92);display:flex;align-items:center;justify-content:center;animation:fadeIn .2s ease;backdrop-filter:blur(12px);}

/* ── Search hint ── */
.search-hint{position:absolute;bottom:calc(100% + 6px);left:0;right:0;background:rgba(18,18,32,.97);border:1px solid rgba(124,58,237,.25);border-radius:12px;padding:8px 12px;font-size:12px;color:rgba(255,255,255,.4);pointer-events:none;animation:fadeInUp .2s ease;}
</style>
</head>
<body>
<div id="root"></div>
...
function CallProvider({children}){
  ...
  const startCall=useCallback(async(targetUser,ct='audio')=>{
    ...
    }catch(e){toast('Нет доступа к микрофону/камере','error');}
  },[createPeer,toast]);
...
function AuthPage(){
  ...
  const submit=async e=>{
    ...
    try{
      ...
      login(d.token,d.user);toast('Добро пожаловать в TeleChat! 🎉','success');
    ...
    <div style={{height:'100vh',width:'100vw',display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,#06060f 0%,#0d0d20 50%,#06060f 100%)',overflow:'hidden',position:'relative'}}>
      {[...Array(20)].map((_,i)=>(
        <div key={i} style={{
          ...
          background:`rgba(${Math.random()>.5?'124,58,237':'167,139,250'},.${Math.floor(Math.random()*5+3)})`,
          animation:`particleFloat ${Math.random()*15+10}s linear infinite`,
          animationDelay:`-${Math.random()*15}s`,pointerEvents:'none',
        }}/>
      ))}
      ...
      <div className="anim-scaleIn" style={{width:'100%',maxWidth:480,padding:'0 20px',maxHeight:'100vh',overflowY:'auto'}}>
        <div className="anim-fadeInDown" style={{textAlign:'center',marginBottom:28}}>
          <div className="float" style={{width:76,height:76,borderRadius:22,background:'linear-gradient(135deg,#7c3aed,#5b21b6)',boxShadow:'0 16px 48px rgba(124,58,237,.5)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 14px'}}>
            <Ic name="msg" size={36} color="white" strokeWidth={1.5}/>
          </div>
          <h1 className="gradient-text" style={{fontSize:34,fontWeight:900,letterSpacing:'-1px'}}>TeleChat</h1>
          <p style={{color:'rgba(255,255,255,.3)',fontSize:13,marginTop:5,fontWeight:500}}>Fast. Secure. Free.</p>
        </div>

        <div className="anim-fadeInUp" style={{background:'rgba(255,255,255,.03)',border:'1px solid rgba(124,58,237,.2)',borderRadius:28,padding:28,backdropFilter:'blur(24px)',boxShadow:'0 24px 80px rgba(0,0,0,.5)',marginBottom:20}}>
          <div style={{display:'flex',background:'rgba(255,255,255,.04)',borderRadius:14,padding:4,marginBottom:24,position:'relative'}}>
            {['login','register'].map(t=>(
              ...
              }}>{t==='login'?'Войти':'Создать аккаунт'}</button>
            ))}
          </div>
          ...
          <form onSubmit={submit}>
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              {tab==='register'&&(
                <>
                  <BigInput label="Имя" type="text" value={form.displayName} onChange={set('displayName')} placeholder="Ваше имя" required/>
                  <BigInput label="Имя пользователя" type="text" value={form.username} onChange={set('username')} placeholder="username (буквы, цифры, _)" required/>
                </>
              )}
              <BigInput label="Email" type="email" value={form.email} onChange={set('email')} placeholder="you@example.com" required/>
              <BigInput label="Пароль" type={showPw?'text':'password'} value={form.password} onChange={set('password')} placeholder={tab==='register'?'Минимум 6 символов':'Ваш пароль'} required
            ...
            {err&&(
              <div className="anim-fadeInDown" style={{marginTop:12,padding:'10px 14px',borderRadius:12,background:'rgba(239,68,68,.1)',border:'1px solid rgba(239,68,68,.25)',color:'#f87171',fontSize:13,fontWeight:500}}>
                ⚠️ {err}
              </div>
            )}

            <button type="submit" disabled={loading} style={{
              width:'100%',marginTop:18,padding:'16px',borderRadius:16,
              ...
              {loading?<span style={{display:'flex',alignItems:'center',justifyContent:'center',gap:10}}><span className="spin" style={{width:18,height:18,border:'2px solid rgba(255,255,255,.3)',borderTopColor:'#fff',borderRadius:'50%',display:'inline-block'}}/> Загрузка...</span>:tab==='login'?'Войти →':'Создать аккаунт →'}
            </button>
          </form>

          <p style={{textAlign:'center',marginTop:18,color:'rgba(255,255,255,.25)',fontSize:13}}>
    ...
    <div>
      ...
      <div style={{position:'relative',display:'flex',alignItems:'center'}}>
        <input {...props} onFocus={e=>{setFocused(true);props.onFocus?.(e);}} onBlur={e=>{setFocused(false);props.onBlur?.(e);}}
          ...
          style={{
            width:'100%',height:56,padding:'0 '+(suffix?'48px':'16px')+' 0 16px',
            ...
            borderRadius:14,color:'#fff',fontSize:15,lineHeight:1.5,
...
function Sidebar({onSelectChat}){
  ...
  const GLOBAL_ID='telechat-global-000-0000-000000000000';
  const filtered=useMemo(()=>{
    let list=search.trim()?chats.filter(c=>(c.name||'').toLowerCase().includes(search.toLowerCase())):chats;
    const global=list.find(c=>c.id===GLOBAL_ID);
    const rest=list.filter(c=>c.id!==GLOBAL_ID);
    return global?[global,...rest]:list;
  },[chats,search]);
  ...
  const getLastMsg=chat=>{
    ...
    if(!chat.last_message) return chat.id===GLOBAL_ID?'👋 Добро пожаловать!':'Начните общение...';
      ...
      <div className="anim-fadeInDown" style={{padding:'16px 16px 12px',borderBottom:'1px solid rgba(255,255,255,.04)'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            <button onClick={()=>setShowProfile(true)} className="avatar-upload-btn" style={{position:'relative',borderRadius:'50%',flexShrink:0,transition:'transform .2s'}} onMouseEnter={e=>e.currentTarget.style.transform='scale(1.08)'} onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}>
              <img src={user?.avatar||`https://ui-avatars.com/api/?name=${encodeURIComponent(user?.displayName||'U')}&background=7c3aed&color=fff&bold=true`}
                alt="" style={{width:40,height:40,borderRadius:'50%',objectFit:'cover',border:'2px solid rgba(124,58,237,.5)',display:'block',boxShadow:'0 0 12px rgba(124,58,237,.3)'}}/>
              <div className="avatar-overlay" style={{borderRadius:'50%'}}><Ic name="camera" size={14} color="#fff"/></div>
        ...
        <div style={{position:'relative'}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)}
            ...
            onFocus2={e=>{e.target.style.borderColor='rgba(124,58,237,.4)';e.target.style.background='rgba(124,58,237,.04)';}}
            onBlur2={e=>{e.target.style.borderColor='rgba(255,255,255,.06)';e.target.style.background='rgba(255,255,255,.05)';}}
          />
      ...
      <div style={{flex:1,overflowY:'auto',padding:'6px 8px'}}>
        {loading?[...Array(5)].map((_,i)=>(
          ...
          <div className="anim-fadeIn" style={{textAlign:'center',padding:'40px 20px',color:'rgba(255,255,255,.2)'}}>
            ...
            <p style={{marginTop:10,fontSize:13}}>{search?'Чатов не найдено':'Нет чатов. Начните общение!'}</p>
          </div>
        ):filtered.map((chat,i)=>{
          const isActive=chat.id===activeChatId;
          const isGlobal=chat.id===GLOBAL_ID;
          return(
            <div key={chat.id} className={`chat-item anim-sidebarItem ${isActive?'active':''}`}
              style={{animationDelay:`${i*.04}s`,display:'flex',alignItems:'center',gap:12,padding:'10px 12px',borderRadius:14,marginBottom:2,cursor:'pointer',
                borderLeft:isActive?'3px solid #7c3aed':'3px solid transparent',
                background:isGlobal&&!isActive?'rgba(124,58,237,.04)':undefined}}
              onClick={()=>{setActiveChatId(chat.id);onSelectChat(chat.id);}}>
              <div style={{position:'relative',flexShrink:0}}>
                {isGlobal?(
                  <div style={{width:46,height:46,borderRadius:'50%',background:'linear-gradient(135deg,#7c3aed,#5b21b6)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,border:`2px solid ${isActive?'rgba(124,58,237,.8)':'rgba(124,58,237,.3)'}`,boxShadow:'0 0 16px rgba(124,58,237,.3)',transition:'all .2s'}}>🌍</div>
                ):(
                ...
                {isGlobal&&<div style={{position:'absolute',bottom:-2,right:-2,width:16,height:16,borderRadius:'50%',background:'linear-gradient(135deg,#7c3aed,#5b21b6)',border:'2px solid #13131f',display:'flex',alignItems:'center',justifyContent:'center',fontSize:8}}>✦</div>}
              </div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:3}}>
                  <div style={{display:'flex',alignItems:'center',gap:5,overflow:'hidden'}}>
                    <p style={{color:isGlobal?'#c4b5fd':'#fff',fontWeight:700,fontSize:14,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{chat.name}</p>
                    {isGlobal&&<span style={{flexShrink:0,fontSize:9,fontWeight:800,color:'#7c3aed',background:'rgba(124,58,237,.15)',border:'1px solid rgba(124,58,237,.3)',borderRadius:6,padding:'1px 5px',letterSpacing:'.5px'}}>GLOBAL</span>}
                  </div>
...
function ProfileModal({onClose}){
  ...
  const saveProfile=async()=>{
    ...
    try{
      ...
      updateUser(d.user);toast('Профиль обновлён!','success');setEdit(false);
  ...
  const handleAvatarChange=async(e)=>{
    ...
    if(file.size>5*1024*1024) return toast('Максимум 5MB','error');
    ...
    try{
      ...
      const r=await fetch('/api/users/avatar',{method:'POST',headers:{Authorization:'Bearer '+token},body:fd});
      const d=await r.json();
      if(!r.ok) throw new Error(d.error||'Upload failed');
      updateUser(d.user); setAvatarPreview(d.user.avatar);
      toast('Аватарка обновлена! ✅','success');
    }catch(err){toast(err.message,'error');setAvatarPreview(user?.avatar||'');}
          ...
          <div style={{position:'relative',display:'inline-block',marginBottom:14}}>
            ...
            <button onClick={()=>fileRef.current?.click()} className="avatar-upload-btn" style={{position:'absolute',inset:0,borderRadius:'50%',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
              <div className="avatar-overlay" style={{borderRadius:'50%',display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
                {uploadingAvatar?<span className="spin" style={{width:20,height:20,border:'2px solid rgba(255,255,255,.3)',borderTopColor:'#fff',borderRadius:'50%',display:'inline-block'}}/>:<><Ic name="camera" size={18} color="#fff"/><span style={{color:'#fff',fontSize:10,fontWeight:700}}>СМЕНИТЬ</span></>}
              </div>
            </button>
        ...
        <div style={{padding:24}}>
          {edit?(
            <div className="anim-fadeIn" style={{display:'flex',flexDirection:'column',gap:14}}>
              <BigInput label="Имя" type="text" value={form.displayName} onChange={e=>setForm(p=>({...p,displayName:e.target.value}))} placeholder="Ваше имя"/>
              ...
              <div>
                <label style={{display:'block',color:'rgba(255,255,255,.4)',fontSize:12,fontWeight:700,marginBottom:6,letterSpacing:'.5px',textTransform:'uppercase'}}>О себе</label>
                <textarea value={form.bio} onChange={e=>setForm(p=>({...p,bio:e.target.value}))} placeholder="Расскажите о себе..." rows={3}
            ...
            <div className="anim-fadeIn" style={{display:'flex',flexDirection:'column',gap:10}}>
              ...
              <InfoRow2 label="Username" value={'@'+user?.username} icon="at"/>
              {user?.bio&&<InfoRow2 label="О себе" value={user?.bio} icon="info"/>}
              <button onClick={()=>setEdit(true)} style={{width:'100%',marginTop:6,padding:'13px',borderRadius:14,background:'linear-gradient(135deg,rgba(124,58,237,.15),rgba(91,33,182,.1))',border:'1px solid rgba(124,58,237,.25)',color:'#a78bfa',fontWeight:700,fontSize:14,transition:'all .2s',display:'flex',alignItems:'center',justifyContent:'center',gap:8}} onMouseEnter={e=>e.currentTarget.style.background='linear-gradient(135deg,rgba(124,58,237,.25),rgba(91,33,182,.2))'} onMouseLeave={e=>e.currentTarget.style.background='linear-gradient(135deg,rgba(124,58,237,.15),rgba(91,33,182,.1))'}>
                <Ic name="edit2" size={15} color="#a78bfa"/> Редактировать профиль
              </button>
            </div>
          )}
        </div>
...
function NewChatModal({onClose}){
  ...
  const createGroup=async()=>{
    if(!groupName.trim()) return toast('Введите название группы','error');
    if(selected.length<1) return toast('Добавьте минимум 1 участника','error');
    setLoading(true);
    ...
    <div className="modal-overlay" style={{position:'fixed',inset:0,zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,.7)',backdropFilter:'blur(8px)',padding:20}} onClick={onClose}>
      <div className="anim-scaleInBounce" style={{width:'100%',maxWidth:400,background:'linear-gradient(135deg,#13131f,#1a1a2e)',border:'1px solid rgba(124,58,237,.2)',borderRadius:28,overflow:'hidden',boxShadow:'0 32px 80px rgba(0,0,0,.7)',maxHeight:'80vh',display:'flex',flexDirection:'column'}} onClick={e=>e.stopPropagation()}>
        <div style={{padding:'20px 20px 16px',borderBottom:'1px solid rgba(255,255,255,.05)'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
            <h3 style={{color:'#fff',fontWeight:800,fontSize:18}}>Новый чат</h3>
        ...
        <div style={{flex:1,overflow:'auto',padding:16}}>
          {tab==='group'&&(
            <div style={{marginBottom:14}}>
              <BigInput label="Название группы" type="text" value={groupName} onChange={e=>setGroupName(e.target.value)} placeholder="Моя крутая группа"/>
            </div>
          )}
          <div style={{position:'relative',marginBottom:12}}>
            ...
            <input value={q} onChange={e=>setQ(e.target.value)}
              ...
              style={{width:'100%',height:42,paddingLeft:36,paddingRight:12,background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.08)',borderRadius:12,color:'#fff',fontSize:14,transition:'all .2s'}}
          ...
          {/* Подсказка про @ */}
          {q===''&&(
            <div className="anim-fadeIn" style={{marginBottom:10,padding:'8px 12px',background:'rgba(124,58,237,.06)',border:'1px solid rgba(124,58,237,.15)',borderRadius:10}}>
              <p style={{color:'rgba(255,255,255,.35)',fontSize:12}}>💡 Введите <span style={{color:'#a78bfa',fontWeight:700}}>@username</span> для поиска по нику</p>
            </div>
          )}
          ...
          <div style={{display:'flex',flexDirection:'column',gap:4}}>
            ...
            {q&&results.length===0&&<p style={{textAlign:'center',color:'rgba(255,255,255,.25)',fontSize:13,padding:'20px 0'}}>Пользователи не найдены по запросу "{q}"</p>}
          </div>
        </div>
        {tab==='group'&&selected.length>0&&(
          <div style={{padding:'12px 16px',borderTop:'1px solid rgba(255,255,255,.05)'}}>
            <button onClick={createGroup} disabled={loading} style={{width:'100%',padding:'13px',borderRadius:14,background:'linear-gradient(135deg,#7c3aed,#5b21b6)',color:'#fff',fontWeight:700,fontSize:15,boxShadow:'0 4px 16px rgba(124,58,237,.35)',transition:'all .2s'}}>
              {loading?'Создаём…':`Создать группу (${selected.length} участников)`}
            </button>
          </div>
        )}
      </div>
...
function ChatWindow({chatId,onBack}){
  ...
  useEffect(()=>{
    const fn=e=>{
      ...
      setMessages(p=>{
        // Не добавляем если уже есть (дубликат своего оптимистичного сообщения)
        if(p.some(x=>x.id===m.id)) return p;
        // Заменяем pending-сообщение если нашли похожее по контенту и отправителю
        const pendingIdx=p.findIndex(x=>x._pending&&x.sender_id===m.sender_id&&x.content===m.content);
        if(pendingIdx!==-1){
          const next=[...p];
          next[pendingIdx]={...m,sender_name:m.sender_name||p[pendingIdx].sender_name,sender_avatar:m.sender_avatar||p[pendingIdx].sender_avatar};
          return next;
        }
        return [...p,m];
      });
    ...
    const fn2=e=>{
      const m=e.detail;
      if(m.chat_id===chatId) setMessages(p=>p.map(x=>x.id===m.id?{...m,sender_name:m.sender_name||x.sender_name,sender_avatar:m.sender_avatar||x.sender_avatar}:x));
  ...
  const send=async()=>{
    ...
    try{
      const d=await POST(`/api/chats/${chatId}/messages`,{type:'text',content:t,reply_to:savedReplyTo?.id||''},token);
      // Заменяем временное сообщение на реальное с сервера
      setMessages(p=>p.map(m=>m.id===tmpId?{...d.message,sender_name:d.message.sender_name||user.displayName||user.username,sender_avatar:d.message.sender_avatar||user.avatar||''}:m));
    }catch(e){
  ...
  const sendFile=async(file)=>{
    ...
    setUploadProgress(`Загружаем ${file.name}…`);
    try{
  ...
  const delMsg=async id=>{
    try{await DEL(`/api/messages/${id}`,token);setMessages(p=>p.map(m=>m.id===id?{...m,deleted:true,content:'Сообщение удалено'}:m));}
    catch(e){toast(e.message,'error');}
  };
  ...
  return(
    <div style={{height:'100%',display:'flex',flexDirection:'column',background:'#0d0d18',position:'relative'}}
      ...
      {isDragOver&&(
        <div style={{position:'absolute',inset:0,zIndex:500,background:'rgba(124,58,237,.15)',border:'3px dashed rgba(124,58,237,.6)',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:16,pointerEvents:'none',backdropFilter:'blur(4px)'}}>
          ...
          <p style={{color:'#a78bfa',fontSize:20,fontWeight:800}}>Перетащите файл для отправки</p>
        </div>
      )}
      ...
      <div className="anim-fadeInDown" style={{padding:'12px 16px',borderBottom:'1px solid rgba(255,255,255,.04)',display:'flex',alignItems:'center',gap:12,background:'rgba(13,13,24,.95)',backdropFilter:'blur(16px)',flexShrink:0}}>
        ...
        <div style={{flex:1,cursor:'pointer'}} onClick={()=>setShowInfo(true)}>
          ...
          <p style={{color:isOnline?'#4ade80':'rgba(255,255,255,.3)',fontSize:12,transition:'color .3s'}}>
            {chat.type==='group'?`${chat.members?.length||0} участников`:isOnline?'В сети':'Не в сети'}
          </p>
        </div>
        <div style={{display:'flex',gap:4}}>
          {chat.type==='private'&&otherUser&&(
            <>
              <HdrBtn onClick={()=>startCall(otherUser,'audio')} title="Аудиозвонок"><Ic name="phone" size={18} color="inherit"/></HdrBtn>
              <HdrBtn onClick={()=>startCall(otherUser,'video')} title="Видеозвонок"><Ic name="video" size={18} color="inherit"/></HdrBtn>
            </>
          )}
          <HdrBtn onClick={()=>setShowInfo(v=>!v)} title="Информация"><Ic name="info" size={18} color="inherit"/></HdrBtn>
        </div>
      </div>
          ...
          if(item.type==='date') return(
            <div key={item.key} className="anim-fadeIn" style={{textAlign:'center',margin:'12px 0 8px'}}>
              <span style={{background:'rgba(255,255,255,.06)',color:'rgba(255,255,255,.3)',fontSize:11,fontWeight:700,padding:'4px 12px',borderRadius:99,backdropFilter:'blur(8px)'}}>
                {new Date(item.date).toLocaleDateString('ru',{weekday:'short',month:'short',day:'numeric'})}
              </span>
            </div>
          );
          const m=item.data;
          const isOwn=m.sender_id===user?.id;
          const isSystem=m.sender_id==='system'||m.is_system;
          const showAvatar=!isOwn&&!isSystem&&(idx===groupedMessages.length-1||groupedMessages[idx+1]?.data?.sender_id!==m.sender_id);
          if(isSystem) return(
            <div key={item.key} className="anim-msgIn" style={{animationDelay:`${Math.min(idx*.02,.3)}s`,display:'flex',justifyContent:'center',marginBottom:12,marginTop:4}}>
              <div style={{background:'rgba(124,58,237,.12)',border:'1px solid rgba(124,58,237,.2)',borderRadius:20,padding:'6px 16px',maxWidth:'80%',textAlign:'center'}}>
                <span style={{color:'rgba(255,255,255,.5)',fontSize:12,fontWeight:500}}>{m.content}</span>
              </div>
            </div>
          );
          return(
            <div key={item.key} className="anim-msgIn msg-row" style={{animationDelay:`${Math.min(idx*.02,.3)}s`,display:'flex',justifyContent:isOwn?'flex-end':'flex-start',marginBottom:2,gap:8,alignItems:'flex-end',position:'relative'}}
              onContextMenu={e=>{e.preventDefault();if(!m.deleted&&!isSystem)setCtxMenu({x:e.clientX,y:e.clientY,msg:m});}}>
              {!isOwn&&(
              ...
              <div style={{maxWidth:'72%'}}>
                {!isOwn&&showAvatar&&(
                  <p style={{color:'#a78bfa',fontSize:11,fontWeight:700,marginBottom:4,marginLeft:4}}>
                    {m.sender_name||m.sender_username||'Пользователь'}
                  </p>
                )}
                ...
                <div className={isOwn?'bubble-own':'bubble-other'} style={{padding:m.file_url&&m.type!=='text'?'6px':'10px 14px',position:'relative',cursor:'default'}}>
                  {m.deleted?(
                    <p style={{color:'rgba(255,255,255,.3)',fontSize:14,fontStyle:'italic'}}>🚫 Сообщение удалено</p>
                  ):(
                    <>
                      {m.file_url&&<FileMessage msg={m}/>}
                  ...
                  {!m.deleted&&(
                    <div style={{display:'flex',alignItems:'center',justifyContent:'flex-end',gap:4,marginTop:m.file_url?4:2}}>
                      {m.edited&&<span style={{color:isOwn?'rgba(255,255,255,.5)':'rgba(255,255,255,.3)',fontSize:11}}>изменено</span>}
                      <span style={{color:isOwn?'rgba(255,255,255,.55)':'rgba(255,255,255,.25)',fontSize:11}}>
                        {new Date(m.created_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}
                      </span>
                      {isOwn&&(m._pending
                        ? <span style={{display:'inline-block',width:10,height:10,border:'1.5px solid rgba(255,255,255,.4)',borderTopColor:'transparent',borderRadius:'50%',animation:'spin .7s linear infinite'}}/>
                        : <Ic name="checks" size={12} color="rgba(255,255,255,.55)"/>
                      )}
                    </div>
                  )}
                </div>
      ...
      {/* Context menu */}
      {ctxMenu&&(
        <div className="anim-scaleIn" style={{position:'fixed',left:Math.min(ctxMenu.x,window.innerWidth-180),top:Math.min(ctxMenu.y,window.innerHeight-200),zIndex:1000,background:'rgba(18,18,32,.98)',border:'1px solid rgba(124,58,237,.2)',borderRadius:16,padding:'6px',boxShadow:'0 16px 48px rgba(0,0,0,.7)',backdropFilter:'blur(16px)',minWidth:170}} onClick={e=>e.stopPropagation()}>
          <CtxBtn icon="reply" color="#a78bfa" label="Ответить" onClick={()=>{setReplyTo(ctxMenu.msg);inputRef.current?.focus();setCtxMenu(null);}}/>
          {ctxMenu.msg.sender_id===user?.id&&<CtxBtn icon="edit2" color="#60a5fa" label="Редактировать" onClick={()=>{setEditing(ctxMenu.msg);setText(ctxMenu.msg.content);inputRef.current?.focus();setCtxMenu(null);}}/>}
          <CtxBtn icon="copy" color="#34d399" label="Копировать" onClick={()=>{navigator.clipboard.writeText(ctxMenu.msg.content);setCtxMenu(null);}}/>
          {ctxMenu.msg.sender_id===user?.id&&<CtxBtn icon="trash" color="#f87171" label="Удалить" onClick={()=>{delMsg(ctxMenu.msg.id);setCtxMenu(null);}} danger/>}
        </div>
      )}

      {/* Input area */}
      <div className="anim-fadeInUp" style={{flexShrink:0,borderTop:'1px solid rgba(255,255,255,.04)',background:'rgba(13,13,24,.95)',backdropFilter:'blur(16px)',padding:'10px 14px'}}>
        {replyTo&&(
          <div className="anim-fadeInDown" style={{display:'flex',alignItems:'center',gap:8,marginBottom:8,padding:'6px 12px',background:'rgba(124,58,237,.08)',borderRadius:12,border:'1px solid rgba(124,58,237,.15)'}}>
            <Ic name="reply" size={14} color="#7c3aed"/>
            <span style={{color:'rgba(255,255,255,.5)',fontSize:12,flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>↩ {replyTo.sender_name}: {replyTo.content}</span>
            <button onClick={()=>setReplyTo(null)} style={{color:'rgba(255,255,255,.3)',transition:'color .2s'}} onMouseEnter={e=>e.currentTarget.style.color='#fff'} onMouseLeave={e=>e.currentTarget.style.color='rgba(255,255,255,.3)'}><Ic name="x" size={14} color="inherit"/></button>
          </div>
        )}
        {editing&&(
          <div className="anim-fadeInDown" style={{display:'flex',alignItems:'center',gap:8,marginBottom:8,padding:'6px 12px',background:'rgba(96,165,250,.08)',borderRadius:12,border:'1px solid rgba(96,165,250,.15)'}}>
            <Ic name="edit2" size={14} color="#60a5fa"/>
            <span style={{color:'rgba(255,255,255,.5)',fontSize:12,flex:1}}>Редактирование сообщения</span>
            <button onClick={()=>{setEditing(null);setText('');}} style={{color:'rgba(255,255,255,.3)',transition:'color .2s'}} onMouseEnter={e=>e.currentTarget.style.color='#fff'} onMouseLeave={e=>e.currentTarget.style.color='rgba(255,255,255,.3)'}><Ic name="x" size={14} color="inherit"/></button>
          </div>
        )}
        {uploading&&(
          <div className="anim-fadeInDown" style={{marginBottom:8,padding:'6px 12px',background:'rgba(124,58,237,.08)',borderRadius:12,border:'1px solid rgba(124,58,237,.15)'}}>
            <span style={{color:'#a78bfa',fontSize:12}}><span className="spin" style={{display:'inline-block',width:12,height:12,border:'2px solid rgba(124,58,237,.3)',borderTopColor:'#7c3aed',borderRadius:'50%',marginRight:6}}/>{uploadProgress}</span>
          </div>
        )}
        <div style={{display:'flex',alignItems:'flex-end',gap:8}}>
          <div style={{position:'relative'}}>
            <button onClick={e=>{e.stopPropagation();setShowEmoji(v=>!v);}} className="icon-btn" style={{width:36,height:36,display:'flex',alignItems:'center',justifyContent:'center',color:'rgba(255,255,255,.3)',borderRadius:10,transition:'all .2s'}} onMouseEnter={e=>e.currentTarget.style.color='#a78bfa'} onMouseLeave={e=>e.currentTarget.style.color='rgba(255,255,255,.3)'}>
              <Ic name="smile" size={20} color="inherit"/>
            </button>
            {showEmoji&&<EmojiPicker onSelect={em=>{setText(p=>p+em);inputRef.current?.focus();}} onClose={()=>setShowEmoji(false)}/>}
          </div>
          <button onClick={()=>fileInputRef.current?.click()} className="icon-btn" style={{width:36,height:36,display:'flex',alignItems:'center',justifyContent:'center',color:'rgba(255,255,255,.3)',borderRadius:10,transition:'all .2s'}} onMouseEnter={e=>e.currentTarget.style.color='#a78bfa'} onMouseLeave={e=>e.currentTarget.style.color='rgba(255,255,255,.3)'}>
          ...
          <textarea ref={inputRef} value={text} onChange={e=>setText(e.target.value)} onKeyDown={handleKey}
            ...
            style={{flex:1,padding:'10px 14px',background:'rgba(255,255,255,.05)',border:'1.5px solid rgba(255,255,255,.07)',borderRadius:18,color:'#fff',fontSize:15,resize:'none',maxHeight:120,lineHeight:1.5,transition:'all .22s'}}
            onFocus={e=>{e.target.style.borderColor='rgba(124,58,237,.4)';e.target.style.background='rgba(124,58,237,.04)';}}
            onBlur={e=>{e.target.style.borderColor='rgba(255,255,255,.07)';e.target.style.background='rgba(255,255,255,.05)';}}
          />
          <button onClick={send} disabled={!text.trim()&&!editing} className="send-btn" style={{
            ...
            boxShadow:text.trim()||editing?'0 4px 16px rgba(124,58,237,.4)':'none',
            transition:'all .25s cubic-bezier(.34,1.56,.64,1)',
          }}>
            <Ic name="send" size={17} color={text.trim()||editing?'#fff':'rgba(255,255,255,.2)'}/>
          </button>
        </div>
      </div>
  ...
  return(
    <div className="anim-slideInRight" style={{width:290,height:'100%',background:'#0f0f1e',borderLeft:'1px solid rgba(124,58,237,.1)',overflow:'auto',flexShrink:0,position:'absolute',right:0,top:0,zIndex:100}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{textAlign:'center',padding:'12px 20px 20px',borderBottom:'1px solid rgba(255,255,255,.05)'}}>
        ...
        {chat.type==='private'&&<p style={{color:chat.other_user?.status==='online'?'#4ade80':'rgba(255,255,255,.3)',fontSize:12,fontWeight:600}}>{chat.other_user?.status==='online'?'🟢 В сети':'⚫ Не в сети'}</p>}
        {chat.type==='group'&&<p style={{color:'rgba(255,255,255,.3)',fontSize:12}}>{chat.members?.length} участников</p>}
      </div>
      {chat.type==='private'&&chat.other_user&&(
        <div style={{padding:'14px 16px',borderBottom:'1px solid rgba(255,255,255,.05)'}}>
          <p style={{color:'rgba(255,255,255,.25)',fontSize:10,fontWeight:800,letterSpacing:'.8px',textTransform:'uppercase',marginBottom:10}}>ИНФОРМАЦИЯ</p>
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            <InfoRow3 label="Username" value={`@${chat.other_user.username}`}/>
            <InfoRow3 label="Email" value={chat.other_user.email}/>
            {chat.other_user.bio&&<InfoRow3 label="О себе" value={chat.other_user.bio}/>}
          </div>
        </div>
      )}
      {chat.type==='group'&&chat.members?.length>0&&(
        <div style={{padding:'14px 16px'}}>
          <p style={{color:'rgba(255,255,255,.25)',fontSize:10,fontWeight:800,letterSpacing:'.8px',textTransform:'uppercase',marginBottom:10}}>УЧАСТНИКИ</p>
    ...
    <div className="anim-slideInUp" style={{position:'fixed',bottom:24,right:24,zIndex:9999,background:'linear-gradient(135deg,rgba(20,20,40,.98),rgba(13,13,30,.98))',border:'1px solid rgba(124,58,237,.4)',borderRadius:24,padding:'18px 20px',boxShadow:'0 24px 80px rgba(0,0,0,.8)',minWidth:290,backdropFilter:'blur(20px)'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:16}}>
        ...
        <div>
          ...
          <p style={{color:'#a78bfa',fontSize:13,fontWeight:600}}>📞 Входящий {callType==='video'?'видео':'аудио'} звонок…</p>
        </div>
      </div>
      <div style={{display:'flex',gap:10}}>
        <button onClick={endCall} style={{flex:1,padding:'11px',borderRadius:14,background:'rgba(239,68,68,.15)',border:'1px solid rgba(239,68,68,.35)',color:'#f87171',fontWeight:700,fontSize:14,transition:'all .2s'}} onMouseEnter={e=>e.currentTarget.style.background='rgba(239,68,68,.25)'} onMouseLeave={e=>e.currentTarget.style.background='rgba(239,68,68,.15)'}>Отклонить</button>
        <button onClick={acceptCall} style={{flex:1,padding:'11px',borderRadius:14,background:'linear-gradient(135deg,#16a34a,#15803d)',border:'none',color:'#fff',fontWeight:700,fontSize:14,boxShadow:'0 4px 16px rgba(22,163,74,.35)',transition:'all .2s'}} onMouseEnter={e=>e.currentTarget.style.transform='scale(1.03)'} onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}>Принять</button>
      </div>
    </div>
  );
  ...
  if(status==='outgoing'||status==='active') return(
    <div style={{position:'fixed',inset:0,zIndex:9000,background:'#060610',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
      ...
      <div className="anim-fadeInUp" style={{textAlign:'center',zIndex:2,marginBottom:20}}>
        ...
        <p style={{color:'rgba(255,255,255,.4)',fontSize:15}}>{status==='outgoing'?'Вызов…':`${callType==='video'?'📹 Видео':'📞 Аудио'} • ${fmt(dur)}`}</p>
      </div>
      <div className="anim-fadeInUp" style={{position:'absolute',bottom:40,display:'flex',gap:18,zIndex:4}}>
        <CallBtn onClick={toggleMute} label={muted?'Включить':'Выкл. микр.'} icon={muted?'micOff':'mic'} active={muted}/>
        {callType==='video'&&<CallBtn onClick={toggleCam} label={camOff?'Камера вкл':'Камера выкл'} icon={camOff?'videoOff':'video'} active={camOff}/>}
        <CallBtn onClick={endCall} label="Завершить" icon="phoneOff" danger/>
      </div>
    </div>
  );
  return null;
  ...
  return(
    <div style={{height:'100%',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'#0d0d18',position:'relative',overflow:'hidden'}}>
      ...
      <div className="anim-scaleIn" style={{display:'flex',flexDirection:'column',alignItems:'center',zIndex:1}}>
        ...
        <h2 className="gradient-text" style={{fontSize:28,fontWeight:900,marginBottom:8,letterSpacing:'-1px'}}>Добро пожаловать в TeleChat</h2>
        <p style={{color:'rgba(255,255,255,.25)',fontSize:14,textAlign:'center',maxWidth:260,lineHeight:1.8}}>Выберите чат или начните новый</p>
        <div style={{display:'flex',gap:20,marginTop:32}}>
          {[['🔒','Безопасно'],['⚡','Реал-тайм'],['📞','HD Звонки'],['📎','Файлы'],['🌍','Глобал чат']].map(([e,t],i)=>(
