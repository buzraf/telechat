...
function ensureGlobalChat(PDO $db): void {
    $check = $db->prepare("SELECT id FROM chats WHERE id=?");
    $check->execute([GLOBAL_CHAT_ID]);
    if ($check->fetch()) return;
    $now = time() * 1000;
    $db->prepare("INSERT INTO chats (id,type,name,description,avatar,created_by,created_at) VALUES (?,?,?,?,?,?,?)")
       ->execute([
           GLOBAL_CHAT_ID,
           'group',
           '🌍 TeleChat Global',
           'Общий чат для всех пользователей TeleChat. Добро пожаловать! 👋',
           'https://ui-avatars.com/api/?name=TeleChat+Global&background=7c3aed&color=fff&bold=true&size=200',
           'system',
           $now
       ]);
    $db->prepare("INSERT INTO messages (id,chat_id,sender_id,type,content,created_at) VALUES (?,?,?,?,?,?)")
       ->execute([
           'msg-welcome-'.time(),
           GLOBAL_CHAT_ID,
           'system',
           'text',
           '👋 Добро пожаловать в TeleChat Global! Это общий чат для всех пользователей. Общайтесь, знакомьтесь и получайте удовольствие! 🚀',
           $now
       ]);
}
...
function addUserToGlobalChat(string $userId, PDO $db): void {
    ...
    $check->execute([GLOBAL_CHAT_ID, $userId]);
    if ($check->fetch()) return;
    $now = time() * 1000;
    ...
    $stmt = $db->prepare("SELECT u.display_name FROM users WHERE id=?");
    $stmt->execute([$userId]);
    $user = $stmt->fetch();
    if ($user) {
        $msgId = 'msg-join-'.time().'-'.substr($userId,0,8);
        $db->prepare("INSERT INTO messages (id,chat_id,sender_id,type,content,created_at) VALUES (?,?,?,?,?,?)")
           ->execute([$msgId, GLOBAL_CHAT_ID, 'system', 'text', '➕ '.$user['display_name'].' присоединился к TeleChat Global!', $now]);
...
if (strpos($path,'/api/')===0) {
    ...
    if ($path==='/api/auth/login'&&$method==='POST') {
        ...
        // Добавляем в глобальный чат если ещё не там
    ...
    if (preg_match('#^/api/chats/([^/]+)/messages$#',$path,$m)&&$method==='GET') {
        ...
        $stmt=$db->prepare("SELECT m.*,u.display_name as sender_name,u.avatar as sender_avatar,u.username as sender_username FROM messages m LEFT JOIN users u ON m.sender_id=u.id WHERE m.chat_id=? AND m.created_at<? ORDER BY m.created_at DESC LIMIT ?");
    ...
    if (preg_match('#^/api/chats/([^/]+)/messages$#',$path,$m)&&$method==='POST') {
        ...
        $stmt=$db->prepare("SELECT m.*,u.display_name as sender_name,u.avatar as sender_avatar,u.username as sender_username FROM messages m LEFT JOIN users u ON m.sender_id=u.id WHERE m.id=?");
    ...
    if (preg_match('#^/api/messages/([^/]+)$#',$path,$m)&&$method==='PUT') {
        ...
        $stmt2=$db->prepare("SELECT m.*,u.display_name as sender_name,u.avatar as sender_avatar,u.username as sender_username FROM messages m LEFT JOIN users u ON m.sender_id=u.id WHERE m.id=?");
...
function Sidebar({onSelectChat}){
  ...
  const getLastMsg=chat=>{
    ...
    if(!chat.last_message) return chat.id===GLOBAL_ID?'👋 Добро пожаловать в общий чат!':'Start chatting...';
          ...
          return(
            <div key={chat.id} className={`chat-item anim-sidebarItem ${isActive?'active':''}`}
              style={{animationDelay:`${i*.04}s`,display:'flex',alignItems:'center',gap:12,padding:'10px 12px',borderRadius:14,marginBottom:2,cursor:'pointer',
                ...
                background:chat.id===GLOBAL_ID&&!isActive?'rgba(124,58,237,.04)':undefined,
                boxShadow:chat.id===GLOBAL_ID&&isActive?'0 0 0 1px rgba(124,58,237,.3) inset':undefined}}
              ...
              <div style={{position:'relative',flexShrink:0}}>
                ...
                {chat.id===GLOBAL_ID&&<div style={{position:'absolute',bottom:-2,right:-2,width:16,height:16,borderRadius:'50%',background:'linear-gradient(135deg,#7c3aed,#5b21b6)',border:'2px solid #13131f',display:'flex',alignItems:'center',justifyContent:'center',fontSize:8}}>✦</div>}
              </div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:3}}>
                  <div style={{display:'flex',alignItems:'center',gap:5,overflow:'hidden'}}>
                    <p style={{color:chat.id===GLOBAL_ID?'#c4b5fd':'#fff',fontWeight:700,fontSize:14,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{chat.name}</p>
                    {chat.id===GLOBAL_ID&&<span style={{flexShrink:0,fontSize:9,fontWeight:800,color:'#7c3aed',background:'rgba(124,58,237,.15)',border:'1px solid rgba(124,58,237,.3)',borderRadius:6,padding:'1px 5px',letterSpacing:'.5px'}}>GLOBAL</span>}
                  </div>
