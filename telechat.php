...
function initDB($db,$t){
  if($t==='pgsql'){
    ...
    try{$db->exec("ALTER TABLE chat_members ADD COLUMN IF NOT EXISTS role VARCHAR(20) DEFAULT 'member'");}catch(Exception $e){}
    try{$db->exec("ALTER TABLE users ADD COLUMN IF NOT EXISTS bio TEXT DEFAULT ''");}catch(Exception $e){}
    try{$db->exec("ALTER TABLE users ADD COLUMN IF NOT EXISTS avatar TEXT DEFAULT ''");}catch(Exception $e){}
    try{$db->exec("ALTER TABLE messages ADD COLUMN IF NOT EXISTS type VARCHAR(20) DEFAULT 'text'");}catch(Exception $e){}
    try{$db->exec("ALTER TABLE messages ADD COLUMN IF NOT EXISTS reply_to BIGINT");}catch(Exception $e){}
    try{$db->exec("ALTER TABLE messages ADD COLUMN IF NOT EXISTS edited BOOLEAN DEFAULT FALSE");}catch(Exception $e){}
