...
// Railway: добавь Volume в настройках проекта с Mount Path /data
// Или используй переменную окружения DATA_DIR
$dbDir = getenv('RAILWAY_VOLUME_MOUNT_PATH') ?: getenv('DATA_DIR') ?: '/data';
if (!is_dir($dbDir) || !is_writable($dbDir)) {
    // fallback — папка рядом со скриптом
    $dbDir = __DIR__ . '/data';
    if (!is_dir($dbDir)) @mkdir($dbDir, 0777, true);
}
if (!is_writable($dbDir)) {
    // последний fallback — /tmp (не сохраняется между деплоями!)
...
function AuthPage() {
  ...
  return (
    <div style={{minHeight:'100vh',width:'100vw',display:'flex',alignItems:'flex-start',justifyContent:'center',background:'linear-gradient(135deg,#06060f 0%,#0d0720 50%,#06060f 100%)',position:'relative',overflowY:'auto',overflowX:'hidden',paddingTop:40,paddingBottom:40}}>
      {/* Animated bg orbs */}
      ...
      <div className="scale-in" style={{width:'100%',maxWidth:480,margin:'0 16px',position:'relative',zIndex:1}}>
        ...
        <div style={{textAlign:'center',marginBottom:14}}>
          <div className="glow-pulse" style={{width:60,height:60,borderRadius:18,background:'linear-gradient(135deg,#7c3aed,#4c1d95)',boxShadow:'0 10px 28px rgba(124,58,237,.5)',display:'inline-flex',alignItems:'center',justifyContent:'center',marginBottom:8}}>
            <Ic name="msg" size={30} color="white" strokeWidth={1.5}/>
          </div>
          <h1 style={{fontSize:26,fontWeight:900,background:'linear-gradient(135deg,#c4b5fd,#fff 50%,#a78bfa)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',letterSpacing:'-1px',marginBottom:2}}>TeleChat</h1>
          <p style={{color:'rgba(255,255,255,.35)',fontSize:12,fontWeight:500}}>Secure. Fast. Beautiful.</p>
        </div>

        {/* Card */}
        <div style={{background:'rgba(255,255,255,.03)',backdropFilter:'blur(24px)',border:'1px solid rgba(124,58,237,.2)',borderRadius:28,padding:'24px 28px',maxHeight:'92vh',overflowY:'auto',boxShadow:'0 24px 80px rgba(0,0,0,.6),inset 0 1px 0 rgba(255,255,255,.08)'}}>
          {/* Tabs */}
          <div style={{display:'flex',background:'rgba(0,0,0,.3)',borderRadius:16,padding:4,marginBottom:14,position:'relative'}}>
          ...
          <form onSubmit={submit}>
            ...
            <button type="submit" disabled={loading} style={{
              width:'100%',marginTop:14,padding:'15px',borderRadius:16,fontSize:17,fontWeight:800,
          ...
          <p style={{textAlign:'center',marginTop:14,color:'rgba(255,255,255,.25)',fontSize:13}}>
...
function BigInput({label,value,onChange,placeholder,type='text',autoComplete,right}) {
  ...
  return (
    <div>
      <label style={{display:'block',color:'rgba(255,255,255,.5)',fontSize:11,fontWeight:700,marginBottom:5,letterSpacing:'.5px',textTransform:'uppercase'}}>{label}</label>
      <div style={{
        ...
        <input value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} type={type} autoComplete={autoComplete}
          ...
          style={{flex:1,padding:'12px 16px',background:'transparent',color:'#fff',fontSize:16,lineHeight:1.5,height:50}}/>
